...
class MainApp:
    def __init__(self):
        ...
        button1 = tk.Button(self.win, text="Lassi", command=lambda: [self.buy_drink("Lassi", 250),self.update_money_in_database()],
                            font=('arial black', 10, 'bold'), bg='blue', fg="#fff", activebackground="maroon",
                            activeforeground="black")
    ...
    def buy_drink(self, drink_name, price):
        if self.total_amount > price:
            try:
                ...
                #update the insert amount
                #self.total_amount
                total = self.total_amount
                query = f"UPDATE money SET Quantity = Quantity + 1 WHERE En = {total}"
                values = (total,)
                ...
                # Update the money table in the database
               ...
               # Update_Money.update_money_in_database()
